...
export async function exportEverything(status: (s: string) => void) {
  ...
  const data = await api.get<{ exported_at: string; tables: Record<string, any[]>; files: { id: number; file_name: string }[]; work_files?: { id: number; file_name: string }[] }>('export');
  ...
  const work: { id: number; name: string; bytes: Uint8Array }[] = [];
  for (const f of data.work_files ?? []) {
    status(`Adding your job files (${work.length + 1} of ${data.work_files!.length})…`);
    const r = await fetch(`/api/ws-items/${f.id}/file`, { credentials: 'same-origin' });
    if (r.ok) work.push({ id: f.id, name: f.file_name, bytes: new Uint8Array(await r.arrayBuffer()) });
  }
  status('Packing the zip…');
  const { buildExportZip } = await import('./exportZip');
  const zip = buildExportZip(data, bytes, names, work);
