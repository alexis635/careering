...
export async function libraryContext() {
  const items = await q<any>(`SELECT id, kind, title, body, tags FROM library_items WHERE deleted_at IS NULL AND NOT ('archived' = ANY(tags)) AND NOT ('generated' = ANY(tags)) ORDER BY kind, id`);
