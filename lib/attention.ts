...
export async function attention() {
  const deadlines = await q(
    `SELECT j.id, j.company, j.role_title, j.stage, to_char(j.deadline,'YYYY-MM-DD') AS deadline, l.name AS lane_name, l.color
      ...
      WHERE j.deadline IS NOT NULL AND j.stage <> 'Closed' AND j.deadline <= current_date + 16
      ORDER BY j.deadline`,
  );
  const stale = await q(
    `SELECT j.id, j.company, j.role_title, j.stage, l.name AS lane_name, l.color,
      ...
      WHERE j.stage IN ('Applied','Screening','Interviewing')
  ...
  const actions = await q(
    `SELECT a.id, a.text, to_char(a.due_date,'YYYY-MM-DD') AS due_date, j.id AS job_id, j.company, j.role_title
       FROM job_actions a JOIN jobs j ON j.id = a.job_id
      WHERE a.done = false AND a.due_date IS NOT NULL AND a.due_date <= current_date + 4
      ORDER BY a.due_date`,
  );
